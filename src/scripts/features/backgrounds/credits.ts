...
export function updateCredits(image?: Background): void {
    ...
    if (image.page.includes('unsplash')) {
        domlocation.href = `${image.page}?utm_source=MyStartupPage&utm_medium=referral`
        domartist.href = `https://unsplash.com/@${image.username}?utm_source=MyStartupPage&utm_medium=referral`
    } else {
